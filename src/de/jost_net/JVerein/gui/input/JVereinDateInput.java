...
/**
 * Fix und fertig konfiguriertes Eingabe-Feld fuer die Datumseingabe. Das ist
 * ein Meta-Input-Feld, welches sich unter der Haube aus einem DialogInput und
 * einem CalendarDialog zusammensetzt.
 * 
 * Im Gegensatz zu DateInput wird er auch beim Aufruf aus einem modlessDialog
 * diesem zugeordnet.
 */
public class JVereinDateInput implements Input
{
   ...
   *          das Datum.
   */
  public JVereinDateInput(Date date)
  {
    this(date, null);
  }
   ...
   *          das Format.
   */
  public JVereinDateInput(Date date, DateFormat format)
  {
    if (format != null)
    {
      this.format = format;
    ...
    this.dialog
        .setText(Application.getI18n().tr("Bitte wählen Sie das Datum aus"));
    this.input = new DialogInput(date == null ? null : this.format.format(date),
        this.dialog);
    this.input.setData(DATAKEY_TOOLTIP,
        String.format(Application.getI18n().tr("DateInput.tooltip")));
    ...
    this.dialog.addCloseListener(new Listener()
    {
      ...
      public void handleEvent(Event event)
      {
        if (event == null || event.data == null)
          return;
        input.setText(JVereinDateInput.this.format.format((Date) event.data));
      }
    });
  }
    ...
    this.input.addKeyListener(new KeyAdapter()
    {
      ...
      public void keyPressed(KeyEvent e)
      {
        ...
        parseAndUpdateInputText().ifPresent(date -> {
          ...
          updatedDate = DateUtil.addMonthsMaintainingEndOfMonth(updatedDate,
              monthOffset);
  ...
  /**
   * Liefert ein Objekt vom Typ {@link Date} oder {@code null}, wenn das Datum
  ...
  private Optional<LocalDate> parseAndUpdateInputText()
  {
    ...
    Optional<LocalDate> date = DateUtil.parseUserInput(inputText,
        this.dtFormatter);
    ...
    if (date.isEmpty())
    {
      Application.getMessagingFactory()
          .sendMessage(new StatusBarMessage(
              Application.getI18n().tr("Ungültiges Datum: {0}", inputText),
              StatusBarMessage.TYPE_ERROR));
      return Optional.empty();
    }

    String parsedText = date.map(d -> d.format(this.dtFormatter))
        .orElse(inputText);
      ...
      catch (Exception e)
      {
        ...
        Application.getMessagingFactory()
            .sendMessage(new StatusBarMessage(Application.getI18n()
                .tr("Ungültiges Datum: {0}", value.toString()),
                StatusBarMessage.TYPE_ERROR));
      }
      return;
    }
  ...
  public void setMandatory(boolean mandatory)
  {
    ...
    if (mandatory != this.mandatory && this.input != null)
    {
      ...
      // TODO Das geht nicht, da update() protecte ist. Ich hab bisher aber auch
      // noch keinen Fehler ohne diese Zeile bemerkt.
      // this.input.update();
    }
    else
    {
