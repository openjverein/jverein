...
public class BuchungsklasseSaldoList extends TablePart implements Part {
	...
	public Part getSaldoList() throws ApplicationException {
		...
		try {
			...
			if (saldoList == null) {
				...
				saldoList.addColumn("Einnahmen", "einnahmen", new CurrencyFormatter("", Einstellungen.DECIMALFORMAT),
						false, Column.ALIGN_RIGHT);
				saldoList.addColumn("Ausgaben", "ausgaben", new CurrencyFormatter("", Einstellungen.DECIMALFORMAT),
						false, Column.ALIGN_RIGHT);
	...
	public ArrayList<BuchungsklasseSaldoZeile> getInfo() throws RemoteException {
		...
		while (buchungsklassenIt.hasNext()) {
			...
			while (buchungsartenSteuerIt.hasNext()) {
				...
				if (buchungsart.getArt() == ArtBuchungsart.EINNAHME) {
					suBukSteuersatz.put(buchungsart.getSteuerBuchungsart().getID(), buchungsart.getSteuersatz());
			...
			while (buchungsartenIt.hasNext()) {
				...
				String sqlc = "select count(*) from buchung, buchungsart " + "where datum >= ? and datum <= ?  "
						+ "and buchung.buchungsart = buchungsart.id " + "and buchungsart.id = ?";
				int anz = (Integer) service.execute(sqlc, new Object[] { datumvon, datumbis, buchungsart.getID() },
						rsi);
				if (anz == 0) {
					continue;
				}
				ausgabe = true;
				String sql = "select sum(betrag) from buchung, buchungsart " + "where datum >= ? and datum <= ?  "
						+ "and buchung.buchungsart = buchungsart.id " + "and buchungsart.id = ? "
						+ "and buchungsart.art = ?";
				einnahmen = (Double) service.execute(sql, new Object[] { datumvon, datumbis, buchungsart.getID(), 0 },
						rsd);
				suBukEinnahmen += einnahmen;
				ausgaben = (Double) service.execute(sql, new Object[] { datumvon, datumbis, buchungsart.getID(), 1 },
						rsd);
				suBukAusgaben += ausgaben;
				umbuchungen = (Double) service.execute(sql, new Object[] { datumvon, datumbis, buchungsart.getID(), 2 },
						rsd);
				...
				if (buchungsart.getSteuersatz() > 0.0) {
					...
					suBukSteuer.put(steuersatz, suBukSteuer.get(steuersatz) + einnahmen + ausgaben + umbuchungen);
				}

				zeile.add(new BuchungsklasseSaldoZeile(BuchungsklasseSaldoZeile.DETAIL, buchungsart, einnahmen,
			...
			zeile.add(new BuchungsklasseSaldoZeile(BuchungsklasseSaldoZeile.SALDOFOOTER,
					"Saldo" + " " + buchungsklasse.getBezeichnung(), suBukEinnahmen, suBukAusgaben, suBukUmbuchungen));
			...
			for (Double steuersatz : suBukNetto.keySet()) {
				...
				if (first_row) {
					zeile.add(new BuchungsklasseSaldoZeile(BuchungsklasseSaldoZeile.STEUERHEADER,
							"Steuern " + buchungsklasse.getBezeichnung(), string_steuersatz, suBukNetto.get(steuersatz),
							suBukSteuer.get(steuersatz)));
					first_row = false;
				} else {
					zeile.add(new BuchungsklasseSaldoZeile(BuchungsklasseSaldoZeile.STEUER, "", string_steuersatz,
		...
		if (einnahmen != 0 || ausgaben != 0 || umbuchungen != 0) {
			...
			zeile.add(new BuchungsklasseSaldoZeile(BuchungsklasseSaldoZeile.DETAIL, "Nicht zugeordnet", einnahmen,
					ausgaben, umbuchungen));
		}
		zeile.add(new BuchungsklasseSaldoZeile(BuchungsklasseSaldoZeile.GESAMTSALDOFOOTER, "Gesamtsaldo" + " ",
				suEinnahmen, suAusgaben, suUmbuchungen));
		zeile.add(new BuchungsklasseSaldoZeile(BuchungsklasseSaldoZeile.GESAMTGEWINNVERLUST, "Gesamt Gewinn/Verlust ",
				suEinnahmen + suAusgaben + suUmbuchungen));
		...
		for (Double steuersatz : suNetto.keySet()) {
			...
			if (first_row) {
				zeile.add(new BuchungsklasseSaldoZeile(BuchungsklasseSaldoZeile.STEUERHEADER, "Gesamtübersicht Steuern",
		...
		sql = "select count(*) from buchung " + "where datum >= ? and datum <= ?  " + "and buchung.buchungsart is null";
